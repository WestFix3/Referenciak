...
func (p *Player) Update() (bomb *Bomb, box *Box, err error) {
	...
	if p.Control.Ability2 && p.State.GetName() == "DetonatorIncrease" && len(p.manualDetonateBombs) > 0 {
	...
	if p.Control.Ability2 && p.State.GetName() == "ObstacleIncrease" && p.State != nil && p.NumberOfObstacles > 0 {
		box = p.PlaceObstacle()
	}
